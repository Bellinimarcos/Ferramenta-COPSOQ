...
with st.expander("Clique aqui para ver as instruções completas", expanded=True):
    ...
    **Prezado(a) Colaborador(a),**
    ...
    - **Confidencialidade:** As suas respostas são **100% confidenciais e anónimas**.
    - **Como Navegar:** A pesquisa está dividida em **8 secções**. Use os botões "Anterior" e "Próximo" para navegar.
    """)
...
# --- Lógica de Navegação por Passos ---
...
for i, (num_pergunta, texto_pergunta) in enumerate(perguntas_do_tema.items()):
    with colunas[i % 2]:
        ...
        resposta = st.radio(
            label=f"**{num_pergunta}.** {texto_pergunta}",

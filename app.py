...
def salvar_dados(lista_de_dados):
    try:
        # Pega as credenciais diretamente da estrutura TOML
        ...
        # Lógica para criar o cabeçalho se a planilha estiver vazia
...
st.markdown("""
    **Prezado(a) Colaborador(a),**
    ...
    Bem-vindo(a) à nossa ferramenta de diagnóstico de riscos psicossociais. Sua participação é um passo muito importante para construirmos, juntos, um ambiente de trabalho cada vez mais saudável e positivo.

    Gostaríamos de assegurar que sua participação é voluntária e que todas as suas respostas são **100% confidenciais e anônimas**. Os resultados serão sempre analisados de forma agrupada, junto com os de outros colegas, e nunca de forma individual. O sigilo da sua percepção pessoal é totalmente garantido.

    **Instruções para o Preenchimento:**

    * Por favor, responda com sinceridade, baseando-se nas suas experiências de trabalho das **últimas 4 semanas**.
    * Não existem respostas "certas" ou "erradas". O que mais importa é a sua opinião honesta.
    * Para cada uma das 84 perguntas, leia atentamente e escolha a opção que melhor descreve sua situação.
    * É necessário responder a todas as perguntas para que o diagnóstico completo seja gerado e o botão "Finalizar" apareça no final da página.

    A sua contribuição é extremamente valiosa. Muito obrigado pela sua participação!
""")
st.divider()
